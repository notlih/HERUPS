...
const Project = () => {
  ...
  function addFavoriteDB(r){
    ...
    let updates = {};
    updates["/users/" + auth.user.displayName + "/favorites/Undoc/" + r.Resource] = r
  ...
  function removeFavoriteDB(r){
    ...
    let updates = {};
    updates["/users/" + auth.user.displayName + "/favorites/Undoc/" + r.Resource] = null
  ...
  useMemo(() => {
    ...
    if(auth.user){   
      ...
      let updates = {};
      updates["/users/" + auth.user.displayName + "/favorites/Undoc/resource"] = 0

      firebase.database().ref().update(updates)
      
      firebase.database().ref("/users/" + auth.user.displayName + "/favorites/Undoc/").once('value').then(function(snapshot){
