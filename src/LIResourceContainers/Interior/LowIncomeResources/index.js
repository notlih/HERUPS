...
const Project = () => {
  ...
  function addFavoriteDB(r){
    ...
    let updates = {};
    updates["/users/" + auth.user.displayName + "/favorites/LowIncome/" + r.Resource] = r
  ...
  function removeFavoriteDB(r){
    ...
    let updates = {};
    updates["/users/" + auth.user.displayName + "/favorites/LowIncome/" + r.Resource] = null
  ...
  useMemo(() => {
    ...
    if(auth.user){   
      ...
      let updates = {};
      updates["/users/" + auth.user.displayName + "/favorites/LowIncome/resource"] = 0

      firebase.database().ref().update(updates)
      
      firebase.database().ref("/users/" + auth.user.displayName + "/favorites/LowIncome/").once('value').then(function(snapshot){
